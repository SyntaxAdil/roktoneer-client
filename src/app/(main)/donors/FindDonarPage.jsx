...
export default function FindDonorPage({
  ...
  const fetchData = async (pageNumber = 1) => {
    setLoading(true);
    try {
      const districtName = bdDistricts.find((d) => d.id === selectedDistrict)?.name || "";
      const upazilaName = bdUpazilas.find((u) => u.id === selectedUpazila)?.name || "";
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/donation-requests/search?${queryParams.toString()}`
      );
    ...
    <section className="w-full py-10 bg-zinc-50 dark:bg-zinc-950 min-h-screen">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-2xl mx-auto mb-12 flex flex-col items-center gap-3">
          ...
          >
            Locate available blood donors instantly.
          </motion.p>
        </div>
          ...
          >
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              <div className="flex flex-col gap-1.5 w-full">
                <label className="text-xs font-bold text-zinc-700 dark:text-zinc-300">Blood Group</label>
                <Select
                  value={selectedBloodGroup || ""}
                  onValueChange={(value) => setValue("bloodGroup", value)}
                >
                  <SelectTrigger className="w-full h-12 rounded-xl bg-zinc-50 dark:bg-zinc-950 border-zinc-200 dark:border-zinc-800 text-left">
                    <SelectValue placeholder="Choose Blood Type" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectGroup>
                      {bloodGroupsInfo.map((item) => (
                        <SelectItem key={item.group} value={item.group}>
                          {item.group}
                        </SelectItem>
                      ))}
                    </SelectGroup>
              ...
              <div className="flex flex-col gap-1.5 w-full">
                <label className="text-xs font-bold text-zinc-700 dark:text-zinc-300">District</label>
                <Select
              ...
              <div className="flex flex-col gap-1.5 w-full">
                <label className="text-xs font-bold text-zinc-700 dark:text-zinc-300">Upazila</label>
                <Select
                  disabled={!districtId}
                  value={selectedUpazila || ""}
                  onValueChange={(value) => setValue("upazila", value)}
                >
                  <SelectTrigger className="w-full h-12 rounded-xl bg-zinc-50 dark:bg-zinc-950 border-zinc-200 dark:border-zinc-800 text-left disabled:opacity-50">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full h-12 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-colors select-none"
              >
            ...
            <div className="flex items-start gap-3 p-4 bg-red-500/5 rounded-2xl border border-red-500/10">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-muted-foreground leading-normal">
                  Search nearby donors instantly for urgent blood requirements.
                </p>
              </div>
            </div>
          </motion.div>
          ...
          >
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center py-20 gap-2">
                ...
                <p className="text-xs text-zinc-400">Searching donors...</p>
              </div>
            ) : !searched ? (
              <NoDonor isInitial />
            ) : requests.length > 0 ? (

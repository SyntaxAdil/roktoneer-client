...
export default function MyRequestsClient({ initialRequests, total, currentPage, currentStatus }) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/donation-requests/${id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className={`w-full max-w-7xl mx-auto px-4 py-8 space-y-8 select-none transition-opacity duration-200 ${isPending ? "opacity-60 pointer-events-none" : "opacity-100"}`}>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Manage your blood requirements board listings.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800/80 rounded-xl px-3 h-10 shadow-sm w-fit">
          ...
          >
            ...
            <SelectContent className="rounded-xl border border-zinc-200/80 dark:border-zinc-800/80 bg-white dark:bg-zinc-950">
              <SelectItem value="all" className="text-xs font-medium rounded-lg">All Status</SelectItem>
              <SelectItem value="pending" className="text-xs font-medium rounded-lg">Pending</SelectItem>
              <SelectItem value="inprogress" className="text-xs font-medium rounded-lg">In Progress</SelectItem>
              <SelectItem value="done" className="text-xs font-medium rounded-lg">Done</SelectItem>
              <SelectItem value="canceled" className="text-xs font-medium rounded-lg">Canceled</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
        ...
        <div className="space-y-8">
          ...
          >
            {initialRequests.map((req) => (
              <MyDonationReqCard key={req._id} req={req} onDelete={handleDelete} />
            ))}
          </motion.div>
          ...
          {total > limit && (
            <div className="flex items-center justify-end gap-2 pt-4 border-t border-zinc-100 dark:border-zinc-900">
              <button
                ...
                onClick={() => updateURL(currentPage - 1, currentStatus)}
              ...
              <button
                disabled={currentPage >= Math.ceil(total / limit) || isPending}
                onClick={() => updateURL(currentPage + 1, currentStatus)}
                className="px-3 h-8 text-xs font-bold border border-zinc-200 dark:border-zinc-800 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-all disabled:opacity-40"
              >
                Next
